...
def KPI_statistics (client, experimentIds, table, it, kpi, unit, exp_type):
    ...
    outliers_yes_no = 0 #0: include outliers, 1: exclude outliers

    ## Connect to the InfluxDB database can be done here or in the main (currently in the main)
    ## client = InfluxDBClient('IP address', port, 'username', 'password', 'db name')
    ## Example: client = InfluxDBClient('triangle.uma.es', 15003, 'username', 'password', 'db name')
    
    rawdf=pd.DataFrame(list(client.query(f"SELECT * FROM \"{table}\" WHERE ExecutionId = \'{experimentIds}\'").get_points()))
    ...
    # Create a dataframe ("Iterations" per column) and evaluate the statistics
...
if __name__ == '__main__':
    ...
    # Include Platform specific credentials here!
    client = InfluxDBClient('IP address', port, 'username', 'password', 'db name')
    ## Example: client = InfluxDBClient('triangle.uma.es', 15003, 'username', 'password', 'db name')
    
    # Mandatory Input examples
    experimentIds = "520"               # Experiment Identifier 
    table = "ADB_Ping_Agent"            # InfluxDB table within the DB "mydb" containing the KPI
    it = "_iteration_"                  # Iteration Identifier
    kpi = "Delay (ms)"                  # KPI Identifier
    unit = 'ms'                         # Measure Unit
    ...
    # Usage in python
    # KPI_statistics(client, experimentIds, table, it, kpi, unit, exp_type)

    # Usage in Command Line
    KPI_statistics(client, *sys.argv[1:])
